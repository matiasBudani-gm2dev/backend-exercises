...
roleRouter.get("/",  authenticateToken, authorizeRoles(["admin"]), async (req, res, next) =>{
    try{
...
roleRouter.get("/:id",  authenticateToken, authorizeRoles(["admin"]), async (req, res, next)=>{
    try{
...
roleRouter.post("/", authenticateToken, authorizeRoles(["admin"]), schemaReqValidation(createRoleSchema) ,async (req, res, next) =>{
    try{
...
roleRouter.put("/:id", authenticateToken, authorizeRoles(["admin"]), schemaReqValidation(updateCompleteRoleSchema),async(req, res, next)=>{
    try{
        const id = Number(req.params.id)
        const {role_name} = req.body
...
roleRouter.patch("/:id",  authenticateToken, authorizeRoles(["admin"]), schemaReqValidation(updatePartialRoleSchema) ,async(req, res, next)=>{
    try{
        const id = Number(req.params.id)
        const {role_name} = req.body
...
roleRouter.delete("/:id",  authenticateToken, authorizeRoles(["admin"]), async(req, res, next)=>{
    try{
        const id = Number(req.params.id)
